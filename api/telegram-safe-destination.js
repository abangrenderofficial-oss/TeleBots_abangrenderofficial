...
export default async function handler(req, res) {
  ...
  // Merely adding the bot to another group must never silently overwrite it.
  if (message) {
  ...
  if (hasMedia(message)) {
    ...
    if (resolved?.isNew) {
      ...
      try {
        ...
        // sees it as known, so the first preview is built locally without the old
        // automatic new-format AI refine path.
...
function identifyMedia(message) {
  if (message.document) return { kind: 'document', fileName: message.document.file_name || '' };
