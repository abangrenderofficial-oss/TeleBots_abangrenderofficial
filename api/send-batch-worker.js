...
export default async function handler(req, res) {
  ...
  while (processed < CHUNK_SIZE) {
    ...
    // HARD GATE immediately before Telegram. A /stop only needs to flip the DB
    // state; old worker invocations cannot send the next item after this check.
    ...
    try {
      ...
      // The batch item itself also stores the destination ID, so reset remains
      // exact even if the dedicated ledger insert has a transient failure.
