...
// Legacy export name is intentionally kept so old imports do not break.
...
export async function maybeAutoNameUntitledDocument({ itemId, chatId }) {
  if (!itemId || !chatId || !process.env.TELEGRAM_BOT_TOKEN || !hasVisionProvider()) return null;
  ...
  if (typeof generatedName !== 'string' || !generatedName.trim()) {
    const result = await nameImageWithVisionFallback(context.file_id).catch((error) => {
