...
export async function sendItem(id, chatId, options = {}) {
  ...
  const recaptionError = String(item.error_message || '');
  const invalidTranslatedRecaption = String(item.status || '').toUpperCase() === 'FAILED'
    && /translation unavailable|translate failed|translation still contains non-latin|recaption validation failed/i.test(recaptionError);
  if (invalidTranslatedRecaption && !forceResend) {
    await debugCallback('send_item_blocked_invalid_translation', {
      item_id: id,
      chat_id: chatId,
      error: recaptionError.slice(0, 500),
    });
    await keepFocus(id);
    await telegram('sendMessage', {
      chat_id: chatId,
      text: '⛔ Item ni belum boleh SEND sebab translation/recaption validation masih gagal. Betulkan atau retry Translate dulu; tajuk asal tak akan dilepaskan ke group sebagai hasil siap.',
    });
    return { ok: false, blocked: 'invalid_translation' };
  }
