...
const SETTING_RETRY_DELAYS_MS = [0, 120, 320];
...
export async function pauseForNewFormat({ chatId, itemId, profileId, signature = null }) {
  ...
  // A partially completed earlier attempt may already have written the review
  // gate but failed before the SEND gate. Always repair the safety gate before
  // trusting or returning an existing format review.
  if (current?.paused) {
    await enforceSendPause(chatId, current, now);
    await pausePipeline({
      chatId,
      reason: PIPELINE_REASONS.NEW_FORMAT,
      itemId: current.item_id,
      sessionId: current.recaption_session_id,
  ...
  const review = {
    ...
    recaption_session_id: item?.recaption_session_id ? String(item.recaption_session_id) : null,
    // Resume only the upload window that was actually paused. Historical
    // PENDING rows from older sessions must never be revived by /resume.
    queue_source_chat_id: item?.source_chat_id != null ? String(item.source_chat_id) : String(chatId),
    queue_start_source_message_id: Number.isFinite(Number(item?.source_message_id))
      ? Number(item.source_message_id)
      : null,
    queue_start_created_at: item?.created_at || now,
    created_at: now,
    updated_at: now,
  };

  // Safety ordering is deliberate: SEND and the unified processing gate must be
  // durable before the owner sees the format-review UI. No worker may launch a
  // later item until explicit double-confirm + /resume clears both gates.
  await enforceSendPause(chatId, review, now);
  await pausePipeline({
    chatId,
    reason: PIPELINE_REASONS.NEW_FORMAT,
    itemId: itemId,
    sessionId: review.recaption_session_id,
    profileId,
    scope: 'chat',
  });
  await setSettingWithRetry(reviewKey(chatId), review);
  return review;
}
...
export async function confirmFormatStep({ chatId, itemId }) {
  ...
  const now = new Date().toISOString();
  await setSetting(`format_profile_confirmed:${review.profile_id}`, {
    confirmed: true,
    confirmed_at: now,
  });
...
async function enforceSendPause(chatId, review, now = new Date().toISOString()) {
  await setSettingWithRetry(`send_paused:${chatId}`, {
    paused: true,
    hard_stop: true,
    reason: 'new_format_review',
    format_item_id: String(review?.item_id || ''),
    format_profile_id: String(review?.profile_id || ''),
    recaption_session_id: review?.recaption_session_id || null,
    paused_at: now,
  });

  // Once the global gate is durable, workers independently observe it before
  // their next Telegram copy. Stopping batch rows as well makes the state
  // explicit and prevents any future continuation kick.
  await stopAllRunningBatches(chatId).catch((error) => {
    console.error('Format review batch-stop persistence failed:', error?.message || error);
    return [];
  });
}

async function setSettingWithRetry(key, value) {
  let lastError = null;
  for (const delayMs of SETTING_RETRY_DELAYS_MS) {
  ...
  throw lastError || new Error(`Unable to persist setting ${key}`);
}
