...
export async function pauseForNewFormat({ chatId, itemId, profileId, signature = null }) {
  ...
  // Safety ordering is deliberate: SEND must be stopped first. If a transient
  // Supabase error happens after this point, the bot can fail the current
  // recaption, but an already-running SEND batch cannot continue into unknown
  // format data. Only after the SEND gate is durable do we publish the review.
