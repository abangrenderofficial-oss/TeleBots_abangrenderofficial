...
export const RECAPTION_CONCURRENCY = 3;
...
export async function runRecaptionSession({ sessionId, workerSecret }) {
  ...
  try {
    const masterGate = await getPipelineState(session.admin_chat_id);
    if (masterGate?.paused) {
      await setRecaptionSessionStatus(session.id, 'PAUSED').catch(() => {});
      return {
        ok: true,
        paused: true,
        pipeline_wait: true,
        pipeline_reason: masterGate.reason,
        status: 'PAUSED',
        session_id: session.id,
      };
    }
    ...
    const candidates = pending.slice(0, RECAPTION_MAX_ITEMS_PER_INVOCATION);
    let processed = 0;
    let pauseObserved = false;
    let aiPauseObserved = false;
    let masterPauseObserved = null;
    let cursor = 0;

    // Processing is parallel in small waves, while media.js keeps Telegram
    // preview delivery ordered by source_message_id. New/unconfirmed formats
    // are treated as a hard boundary and are processed alone before anything
    // after them can start.
    while (cursor < candidates.length) {
      if (Date.now() - startedAt >= RECAPTION_MAX_WORK_MS) break;

      const livePipeline = await getPipelineState(session.admin_chat_id);
      if (livePipeline?.paused) {
        masterPauseObserved = livePipeline;
        break;
      }
      ...
      const next = await takeNextRecaptionWave(candidates, cursor);
      ...
      const results = await Promise.all(wave.map(async (item) => {
        try {
          ...
          if (result?.paused_new_format) {
            return { item, result, pause_observed: true };
          }

          // Vision naming can discover AI exhaustion inside processPendingItem.
          // It marks the item back to PENDING and stores a durable AI wait state.
          const aiWait = await getRecaptionAiWait(session.id);
          if (aiWait?.paused) {
            return { item, result, ai_pause_observed: true };
          }
      ...
      for (const entry of results) {
        if (entry.pause_observed || entry.result?.paused_new_format) {
          pauseObserved = true;
        }
        if (entry.ai_pause_observed) {
          aiPauseObserved = true;
        }

        if (!entry.error) continue;
        const errorText = String(entry.error?.message || entry.error).slice(0, 1000);

        if (isAiUnavailableError(entry.error)) {
          await pauseRecaptionForAi({
            sessionId: session.id,
            chatId: session.admin_chat_id,
            itemId: entry.item.id,
            kind: /vision unavailable/i.test(errorText) ? 'vision' : 'translation',
            error: entry.error,
          }).catch((pauseError) => {
            console.error('AI wait pause failed:', pauseError?.message || pauseError);
          });
          aiPauseObserved = true;
          continue;
        }

        await updateQueueItem(entry.item.id, {
          status: 'FAILED',
          error_message: errorText,
        }).catch(() => {});
        console.error('Manual recaption item failed:', entry.item.id, errorText);
      }

      const durableAiWait = await getRecaptionAiWait(session.id);
      if (durableAiWait?.paused) aiPauseObserved = true;

      const afterWavePipeline = await getPipelineState(session.admin_chat_id);
      if (afterWavePipeline?.paused) masterPauseObserved = afterWavePipeline;

      // Do not launch a later wave after any safety gate has been raised.
      if (pauseObserved || aiPauseObserved || masterPauseObserved) break;
    }

    if (masterPauseObserved) {
      await setRecaptionSessionStatus(session.id, 'PAUSED').catch(() => {});
      return {
        ok: true,
        paused: true,
        pipeline_wait: true,
        pipeline_reason: masterPauseObserved.reason,
        processed,
        status: 'PAUSED',
        session_id: session.id,
      };
    ...
    if (!remaining.length) {
      ...
      await clearRecaptionAiState(session.id).catch(() => {});
      if (finalStatus === 'COMPLETED') await releaseOwnedSendGate(session).catch(() => {});

      await telegram('sendMessage', {
        chat_id: session.admin_chat_id,
        text: failed.length
          ? `⚠️ RECAPTION selesai dengan ${failed.length} gagal · ${Math.max(0, Number(session.item_count || 0) - failed.length)} processed.`
...
async function takeNextRecaptionWave(items, startIndex) {
  const wave = [];
  let nextIndex = startIndex;

  while (nextIndex < items.length && wave.length < RECAPTION_CONCURRENCY) {
    const item = items[nextIndex];
    let needsReview = false;

    try {
      const profile = await getProfileForItem(item);
      const confirmedMarker = profile?.id
        ? await getSetting(`format_profile_confirmed:${profile.id}`).catch(() => null)
        : null;
      needsReview = Boolean(!profile || (!profile.learned && !confirmedMarker?.confirmed));
    } catch (error) {
      // Let the normal item processor own the actual error path. Keeping this
      // item alone avoids launching later items past an uncertain boundary.
      console.error('Recaption wave preflight failed:', item?.id, error?.message || error);
      needsReview = true;
    }
