...
export async function sendFormatManagerList(chatId, options = {}) {
  ...
  const text = profiles.length
    ? '🧠 FORMAT MANAGER\n\nPilih mana-mana format lama untuk betulkan setting dia. Semua perubahan auto-save terus untuk format itu sahaja.'
...
export async function showFormatManager(profileId, chatId, messageId = null) {
  ...
  const text = [
    ...
    '',
    `Status: ${status}`,
    'Setting bawah ni milik format ini sahaja. Tekan option untuk betulkan; perubahan terus auto-save.',
    '♻️ APPLY CURRENT BATCH hanya update item matching yang belum SENT dalam recaption batch terbaru.',
  ].join('\n');
  ...
  const rows = [
    ...
    [
      { text: '♻️ APPLY CURRENT BATCH', callback_data: `fmtmgr_applybatch:${profile.id}` },
    ],
    [
      { text: '✅ SIAP', callback_data: 'fmtmgr_page:0' },
    ],
  ];
