...
export async function recaptionItemWithProfile(itemId, profile, options = {}) {
  ...
  try {
    ...
    // the multi-provider router only when the extracted title actually contains
    // a source script that needs translation.
    ...
    translationRequired = Boolean(translateEnabled && titleNeedsTranslation(baseProcessed));
    aiBypassed = Boolean(
      translationRequired
...
async function translateProcessedTitle(processed) {
  ...
  // English/Latin titles do not spend any translation quota.
  if (!coreTitle || !hasNonLatinSourceScript(coreTitle)) return processed;
