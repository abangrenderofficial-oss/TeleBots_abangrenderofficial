...
import { withTranslationSlot } from '../../ai-concurrency.js';
...
export async function recaptionItemWithProfile(itemId, profile, options = {}) {
  ...
  const translateEnabled = Boolean(profile?.actions?.translate);
  let translationRequired = false;
  let aiBypassed = false;

  try {
    // Title extraction stays deterministic/local. Translation is handled by
    // the multi-provider router after the title/serial shape is canonicalized.
    ...
    // A format can extract the same serial both as the dedicated No Siri value
    // and again inside the title text. Canonicalize it before any translation
    // so downstream logic always sees exactly one serial line.
    if (profile?.actions?.take_serial) {
      baseProcessed = normalizeCanonicalSerial(baseProcessed);
    }

    translationRequired = Boolean(
      translateEnabled
      && hasTranslatableCore(baseProcessed)
      && (options.forceTranslateAllLanguages || titleNeedsTranslation(baseProcessed)),
    );
    aiBypassed = Boolean(
      translationRequired
      && (
        options.aiBypass
        || (
          item.recaption_session_id
          && await isRecaptionAiBypass(item.recaption_session_id)
        )
      ),
    );

    if (translationRequired && !aiBypassed) {
    ...
    const validation = validateProcessedAgainstProfile(processed, profile, item, {
      allowUntranslated: aiBypassed,
    });
    if (!validation.ok) {
      throw new Error(`Recaption validation failed: ${validation.errors.join('; ')}`);
    }

    const nextStatus = options.keepPending
      ? item.status
      : (item.status === 'SENT' ? 'SENT' : 'READY');
    ...
    await setSetting(`recaption_last:${item.id}`, {
      ...
      used_ai: Boolean(translationRequired && !aiBypassed),
      ai_required: translationRequired,
      ai_bypassed: aiBypassed,
      force_translate_all_languages: Boolean(options.forceTranslateAllLanguages),
    ...
    // Batch sessions are handled centrally by recaption-runner so the user gets
    // one pause + Continue Without AI button instead of repeated warnings.
    if (
      !item.recaption_session_id
      && translationRequired
      && /translation unavailable|translate failed|translation still contains non-latin/i.test(message)
    ) {
...
function hasTranslatableCore(processed) {
  ...
  if (String(lines[0] || '').trim().toLowerCase() !== serial.toLowerCase()) return true;
  return Boolean(lines.slice(1).join('\n').trim());
}
...
async function translateProcessedTitle(processed) {
  ...
  // The caller decides whether translation is needed. In strict immediate mode
  // every titled item with Translate ON passes through the translator so Latin
  // script languages (Spanish/French/etc.) are not missed by script heuristics.
  const translation = await withTranslationSlot(() => translateTitleFailover(coreTitle));
...
export function normalizeCanonicalSerial(processed) {
  const serial = String(processed?.serial || '').trim();
  const fullTitle = String(processed?.title || '').trim();
  if (!serial || !fullTitle) return processed;

  const serialPattern = new RegExp(
    `(^|\\s)${escapeRegExp(serial)}(?=$|\\s|[|:;,])`,
    'gi',
  );
  const coreTitle = fullTitle
    .split(/\r?\n/)
    .map((line) => line
      .replace(serialPattern, '$1')
      .replace(/^[\s|:;,-]+/, '')
      .replace(/[ \t]{2,}/g, ' ')
      .trim())
    .filter(Boolean)
  ...
  const normalizedTitle = [serial, coreTitle]
    .filter(Boolean)
    .join('\n')
    .trim()
    .slice(0, 220);

  if (normalizedTitle === fullTitle) return processed;

  const oldWrapped = `<b>${escapeHtml(fullTitle)}</b>`;
  const newWrapped = `<b>${escapeHtml(normalizedTitle)}</b>`;
  let finalCaptionHtml = String(processed?.finalCaptionHtml || '');
  if (finalCaptionHtml.includes(oldWrapped)) {
    finalCaptionHtml = finalCaptionHtml.replace(oldWrapped, newWrapped);
  } else if (/^<b>[\s\S]*?<\/b>/.test(finalCaptionHtml)) {
    finalCaptionHtml = finalCaptionHtml.replace(/^<b>[\s\S]*?<\/b>/, newWrapped);
  } else if (!finalCaptionHtml) {
    finalCaptionHtml = newWrapped;
  }
...
export function validateProcessedAgainstProfile(processed, profile, item = {}, options = {}) {
  const errors = [];
  const actions = {
    take_title: true,
    take_serial: false,
  ...
  if (actions.take_serial) {
    ...
    if (expectedSerial && countSerialOccurrences(title, expectedSerial) > 1) {
      errors.push('canonical serial appears more than once');
    }
  }
...
function countSerialOccurrences(title, serial) {
  const pattern = new RegExp(
    `(^|\\s)${escapeRegExp(serial)}(?=$|\\s|[|:;,])`,
    'gi',
  );
  return Array.from(String(title || '').matchAll(pattern)).length;
}
