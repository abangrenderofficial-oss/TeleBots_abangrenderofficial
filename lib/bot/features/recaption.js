...
export async function recaptionItemWithProfile(itemId, profile, options = {}) {
  ...
  try {
    ...
    // is handled separately through the provider failover router below.
    ...
    if (needsAi && /translation unavailable|translate failed/i.test(message)) {
      await telegram('sendMessage', {
        ...
        text: '⚠️ Translate gagal sementara sebab semua provider translation tengah busy/timeout. Tajuk asal TAK dianggap berjaya translate dan tak akan dicache. Cuba Translate semula sekejap lagi.',
      }).catch(() => {});
    }
...
async function translateProcessedTitle(processed) {
  ...
  const translatedCore = translation.text;
