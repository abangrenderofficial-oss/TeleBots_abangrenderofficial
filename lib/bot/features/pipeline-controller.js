...
const PREP_START_REVISION_PREFIX = 'immediate_prep_revision_v1:';
...
function prepStartRevisionKey(itemId) {
  return `${PREP_START_REVISION_PREFIX}${itemId}`;
}
...
export async function pausePipeline({
  ...
  // Never let another concurrent worker steal ownership of an already-raised
  // safety gate. The first NEW_FORMAT / MANUAL_STOP / AI_LIMIT boundary remains
  // authoritative until the matching explicit resume path clears it.
  if (
    current?.paused
    && (
      [PIPELINE_REASONS.NEW_FORMAT, PIPELINE_REASONS.MANUAL_STOP].includes(current.reason)
      || (current.reason === PIPELINE_REASONS.AI_LIMIT && reason === PIPELINE_REASONS.AI_LIMIT)
    )
  ) {
    return current;
  }
...
// Immediate worker calls this immediately after resolving the item's format and
// before any translation/vision work. Besides returning the scoped bypass flag,
// snapshot the profile revision at prep START. If /formats changes that profile
// while this item is still preparing, the sender later sees an old prepared
// revision and forces a clean re-prepare instead of leaking half-old settings.
export async function isDirectAiBypass(chatId, itemId) {
  const [value, context] = await Promise.all([
    getSetting(directAiBypassKey(chatId)).catch(() => null),
    getSetting(`${ITEM_CONTEXT_PREFIX}${itemId}`).catch(() => null),
  ]);

  if (context?.profile_id) {
    const revision = await getFormatProfileRevision(context.profile_id);
    await setSetting(prepStartRevisionKey(itemId), {
...
export async function stampPreparedProfileRevision(itemId, profileId, revision = null) {
  const [snapshot, current] = await Promise.all([
    getSetting(prepStartRevisionKey(itemId)).catch(() => null),
    getFormatProfileRevision(profileId),
  ]);
  const supplied = Number(revision);
  const snapshotRevision = (
    String(snapshot?.profile_id || '') === String(profileId || '')
    && Number.isFinite(Number(snapshot?.revision))
  ) ? Number(snapshot.revision) : null;
  const effectiveRevision = Number.isFinite(supplied) && supplied > 0
    ? Math.floor(supplied)
    : (snapshotRevision != null ? Math.floor(snapshotRevision) : current);

  await Promise.all([
    setSetting(preparedRevisionKey(itemId), {
      profile_id: String(profileId || ''),
      revision: effectiveRevision,
      prepared_at: new Date().toISOString(),
    }),
    setSetting(prepStartRevisionKey(itemId), null),
  ]);
...
export async function clearPreparedProfileRevision(itemId) {
  await Promise.all([
    setSetting(preparedRevisionKey(itemId), null),
    setSetting(prepStartRevisionKey(itemId), null),
  ]);
  return true;
}
