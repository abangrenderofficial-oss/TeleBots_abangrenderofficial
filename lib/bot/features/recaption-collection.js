...
export async function collectForwardedQueueItem({ message, media }) {
  ...
  if (!result?.item?.id) throw new Error('Collected item tak lengkap');
  if (!result.replay && !result.session_id) throw new Error('Collected session tak lengkap');
  return result;
}
...
export async function claimRecaptionWorker(sessionId, workerSecret, leaseToken) {
  return rpc('claim_recaption_worker', {
...
export async function releaseRecaptionWorker(sessionId, workerSecret, leaseToken) {
  return rpc('release_recaption_worker', {
    p_session_id: String(sessionId),
    p_worker_secret: String(workerSecret),
    p_lease_token: String(leaseToken),
  });
...
export async function getLatestRecaptionSession(chatId, statuses = ['PROCESSING', 'PAUSED']) {
  const clean = (Array.isArray(statuses) ? statuses : [statuses])
    .map((status) => String(status || '').toUpperCase())
    .filter((status) => ['COLLECTING', 'PROCESSING', 'PAUSED', 'COMPLETED', 'FAILED'].includes(status));
  if (!clean.length) return null;
  const rows = await request(
    `${table('recaption_sessions')}?admin_chat_id=eq.${encodeURIComponent(chatId)}`
    + `&status=in.(${clean.join(',')})&order=created_at.desc&limit=1&select=*`,
  );
  return rows?.[0] || null;
}
