...
export async function handle({ message, res }) {
  await sendText(message.chat.id, 'Guna /connect dalam group target yang kau nak jadikan destination SEND.');
  return res.status(200).json({ ok: true, command: 'connect', connected: false });
}
