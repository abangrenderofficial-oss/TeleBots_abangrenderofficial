...
export async function handle({ message, res }) {
  ...
  if (existing) {
    ...
    const runnable = status === 'PROCESSING'
      ? existing
      : await setRecaptionSessionStatus(existing.id, 'PROCESSING');

    await sendText(
      chatId,
      status === 'PAUSED'
        ? `▶️ RECAPTION recovery · sambung semula ${existing.item_count || 0} item.`
        : `⏳ RECAPTION memang masih berjalan · ${existing.item_count || 0} item. Aku kick worker sekali lagi untuk pastikan ia tak tersangkut.`,
    );
    waitUntil(kickRecaptionWorker(runnable || existing).catch(async (error) => {
      console.error('Recaption recovery kick failed:', error?.message || error);
      await sendText(chatId, `❌ Worker masih tak dapat start. Session tak hilang; /recaption boleh retry lagi. ${String(error?.message || error).slice(0, 180)}`).catch(() => {});
    }));
    ...
    return res.status(200).json({
      ...
      status: runnable?.status || existing.status,
    });
  }
  ...
  waitUntil(kickRecaptionWorker(session).catch(async (error) => {
    ...
    await sendText(chatId, `❌ RECAPTION worker tak dapat start. Session kekal selamat; hantar /recaption untuk retry. ${String(error?.message || error).slice(0, 180)}`).catch(() => {});
  }));
