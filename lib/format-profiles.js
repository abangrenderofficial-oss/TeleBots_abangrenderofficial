...
async function extractTitleForProfile({ caption, fileName, translate, removeHashtags }) {
  ...
  if (!apiKey) {
    ...
    if (!translate) titleMemory.set(cacheKey, value);
    return value;
  }
...
Return title text only. Maximum two lines.`;
  ...
  try {
    ...
    const value = localTitle(cleanedCaption, fileName);
    // A failed translation must stay retryable. Caching the untranslated local
    // fallback here caused every later Translate press to return the same title
    // for the lifetime of a warm serverless instance.
    if (!translate) titleMemory.set(cacheKey, value);
    return value;
  }
}
...
async function callGeminiText(system, prompt) {
  ...
  for (const model of models) {
    try {
      ...
      errors.push(`${model}: ${error?.name === 'AbortError' ? 'timeout' : (error?.message || 'request failed')}`);
    }
  }
