...
export async function resolveFormatProfile({ caption = '', fileName = '', mediaKind = 'other' }) {
  ...
  const globalFooter = await getSetting('caption_footer_html');
...
export async function saveItemFormatContext(itemId, context) {
  if (!itemId) return;
  return setSetting(`${ITEM_CONTEXT_PREFIX}${itemId}`, {
    ...context,
    updated_at: new Date().toISOString(),
  });
}
...
export async function getItemFormatContext(itemId) {
  ...
  return getSetting(`${ITEM_CONTEXT_PREFIX}${itemId}`);
}
...
export async function toggleFormatOption(profileId, option) {
  ...
  // Translate only makes sense when a title is being extracted.
...
export async function processMediaWithProfile({ caption = '', fileName = '', profile }) {
  ...
  const footerHtml = actions.add_footer
    ? (p.footer_html || (await getSetting('caption_footer_html')) || '')
    : '';
...
async function extractTitleForProfile({ caption, fileName, translate, removeHashtags }) {
  ...
  if (!apiKey) return localTitle(cleanedCaption, fileName);
...
Return title text only. Maximum two lines.`;
  ...
  try {
    ...
    return cleanTitle(text) || localTitle(cleanedCaption, fileName);
  } catch (error) {
...
async function saveProfiles(profiles) {
  return setSetting(PROFILE_KEY, (profiles || []).map(normalizeProfile).slice(-120));
}
