...
export async function agentAssistant({ chatId, text, focusedItemId = null }) {
  ...
  if (!apiKey) {
    return fallbackPlan(text, context);
  }
  ...
  fallback.reply = fallback.reply || `Aku faham mesej kau, tapi AI tengah lambat sekejap. Cuba sekali lagi. (${safeError(lastError)})`;
...
function parsePlan(raw) {
  try {
    ...
    const action = allowed.has(data.action) ? data.action : 'chat';
    return {
      action,
...
function fallbackPlan(text, context) {
  ...
  if (/\b(total|berapa|stat|jumlah|file|fail)\b/.test(t) && /\b(total|berapa|stat|jumlah|sent|pending|failed|hantar|belum)\b/.test(t)) {
    ...
    return {
      action: 'show_stats',
      reply: `Sekarang ada ${s.total} file. ${s.sent} dah dihantar, ${s.pending} masih pending dan ${s.failed} failed.`,
      title: '', learnRule: '', memory: '',
    };
  }
  if (/\b(pending|belum hantar|tak hantar|unsent)\b/.test(t)) {
    return { action: 'show_pending', reply: 'Aku tunjuk item yang masih pending.', title: '', learnRule: '', memory: '' };
  }
  if (/\b(send|hantar|publish|post)\b/.test(t) && context.item) {
    return { action: 'preview_item', reply: 'Aku sediakan preview item ni. Kalau dah okay, tekan SEND.', title: '', learnRule: '', memory: '' };
  }
