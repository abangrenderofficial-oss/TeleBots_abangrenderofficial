...
- revise_item: owner wants the current item cleaned, translated, renamed, reformatted, hashtags removed, serial/model retained, or caption/title corrected. Put only the final product/model title in title. Never put the saved footer inside title.
...
- If a format is unfamiliar or confidence is low, say so plainly and ask the owner how it should be handled. Never pretend you learned a format that is not represented in context.
...
{"action":"chat|show_stats|show_pending|preview_item|revise_item|remember|skip_item","reply":"natural reply","title":"","learn_rule":"","memory":""}`;
...
export async function agentAssistant({ chatId, text, focusedItemId = null }) {
  ...
  const historyText = (context.history || [])
    .slice(-6)
    .map((m) => `${m.role === 'assistant' ? 'Assistant' : 'Owner'}: ${String(m.content || '').slice(0, 700)}`)
    .join('\n');

  const prompt = `OWNER MESSAGE:\n${text}\n\nRECENT CHAT:\n${historyText || '(none)'}\n\nCURRENT CONTEXT:\n${context.contextText}`;

  // Keep Telegram replies fast. One low-latency model call, then a local fallback.
...
function parsePlan(raw) {
  try {
    ...
    const allowed = new Set(['chat', 'show_stats', 'show_pending', 'preview_item', 'revise_item', 'remember', 'skip_item']);
    return {
...
function fallbackPlan(text, context) {
  ...
  if (/(faham tak|faham ke|understand|kau faham|jelas tak)/i.test(t)) {
    return plan('chat', item ? 'Faham. Kau boleh terus sembang biasa pasal item ni. Kalau nak ubah apa-apa, cakap je dan aku akan buat preview baru.' : 'Faham. Kau boleh sembang biasa je dengan aku, tak perlu ikut format command tertentu.');
  }
  ...
  if (item && /(buang|remove|delete|padam|ambil|keep|kekal|translate|terjemah|tajuk|title|hashtag|serial|siri|model|caption|rename|tukar)/i.test(t)) {
...
function plan(action, reply) {
  return { action, reply, title: '', learnRule: '', memory: '' };
}
