...
export async function telegram(method, payload = {}) {
  ...
  if (!editedExistingPreview) {
    if (method === 'copyMessage' && !isOwnerPreview(payload)) {
      result = await idempotentDestinationCopy(token, payload);
    } else {
  ...
  // any extra controls. A duplicate webhook/callback that merely reuses an
  // already-sent destination message must not increment this record again.
  if (method === 'copyMessage' && !isOwnerPreview(payload) && !result?.__deduped) {
...
async function updateLastSentPreviewStatus(token, payload) {
  ...
  // increase the number; only Telegram documents/files do.
