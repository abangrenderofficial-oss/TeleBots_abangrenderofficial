import { getQueueItem, getQueueItemBySourceMessage, getSetting, setSetting, stats } from './store.js';
...
async function updateLastSentPreviewStatus(token, payload) {
  ...
  // Count only real files/documents. Photos/images never increase this record.
  // This hook runs right after Telegram accepts the copy, just before sendItem()
  // updates the current row to SENT, so include the current document once here.
  const fileStats = await stats().catch(() => null);
  let sentFileTotal = Number(fileStats?.sent || 0);
  ...
  if (item.preview_message_id) {
    await rawTelegram(token, 'editMessageReplyMarkup', {
      ...
      reply_markup: {
        inline_keyboard: [[{ text: `🏁 LAST SENT · FILE ${sentFileTotal}`, callback_data: 'noop' }]],
      },
    }).catch((error) => {
