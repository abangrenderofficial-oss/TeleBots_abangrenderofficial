...
export async function telegram(method, payload = {}) {
  ...
  // at the stable production alias, never VERCEL_URL (which is deployment-specific
  // and can leave the bot stuck on an old immutable deployment after a new push).
...
function getStableWebhookUrl() {
  ...
  const projectProduction = String(process.env.VERCEL_PROJECT_PRODUCTION_URL || '').trim();
  if (projectProduction) {
    const host = projectProduction.replace(/^https?:\/\//i, '').replace(/\/+$/, '');
    return `https://${host}/api/telegram`;
  }

  return STABLE_WEBHOOK_URL;
}
