...
export async function inspectIncomingDuplicate({
  ...
  if (fileUniqueId) {
    ...
    // Only return the auto-delete kind when this exact Telegram file has
    // already completed a successful SEND to the destination/group.
    // sendItem sets status=SENT only after Telegram copyMessage succeeds.
