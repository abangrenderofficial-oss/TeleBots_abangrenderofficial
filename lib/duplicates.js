...
export async function inspectIncomingDuplicate({
  ...
  // A top-level duplicate review gate may explicitly allow one exact duplicate
  // occurrence to pass through the normal caption/preview pipeline.
  ...
  if (fileUniqueId) {
    ...
    // Exact files that were already SENT must NEVER trigger the old auto-delete
    // path. The top-level duplicate review gate detects these directly from DB,
    // lets the new copy become a normal editable preview, and then shows one
    // grouped duplicate summary. Returning "none" here guarantees no lower
    // legacy handler can delete the owner's newly forwarded photo/file/caption.
    ...
    if (alreadySent) {
      return {
        ...
        match: alreadySent,
        confidence: 1,
        exact_already_sent: true,
        reason: 'Telegram file_unique_id sama dan item asal sudah berjaya SENT; keep new copy for review.',
      };
    }
...
export function duplicateNotice(result) {
  ...
  // Kept only for compatibility with old stored/debug results. New live exact
  // SENT duplicates never reach this branch because inspectIncomingDuplicate()
  // returns kind:none for them.
