...
async function callProvider(provider, key, model, input) {
  ...
  try {
    ...
    if (!response.ok) {
      return { text: '', meta: { ok: false, status: response.status, error: errorText(data) || `HTTP ${response.status}`, elapsed_ms: Date.now() - started } };
    }

    const text = extractText(data);
    return { text, meta: { ok: Boolean(text), status: response.status, error: text ? null : 'empty response', elapsed_ms: Date.now() - started } };
  } catch (error) {
...
function clean(value) {
  return String(value || '').replace(/^```(?:text)?\s*/i, '').replace(/```$/i, '').replace(/^(?:translation|translated title|title)\s*:\s*/i, '').trim().slice(0, 300);
}
