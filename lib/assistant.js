...
export async function chatAssistant({ chatId, text }) {
  ...
  if (!apiKey) {
    return 'Gemini belum configured untuk AI chat.';
  }
  ...
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(model)}:generateContent`,
    {
      ...
      body: JSON.stringify({
        systemInstruction: {
          parts: [{ text: `${BASE_SYSTEM}${runtimeContext}` }],
        },
        contents,
        generationConfig: {
          temperature: 0.45,
  ...
  if (!response.ok) {
    throw new Error(`Gemini chat failed: ${JSON.stringify(data)}`);
  }
  ...
  const clean = (answer || 'Aku tak dapat hasilkan jawapan sekarang. Cuba sekali lagi.').slice(0, 3900);
