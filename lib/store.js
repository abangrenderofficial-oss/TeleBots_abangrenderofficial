...
export async function recentChatMessages(adminChatId, limit = 18) {
  const rows = await request(
    `${table('ai_chat_messages')}?admin_chat_id=eq.${encodeURIComponent(adminChatId)}&order=created_at.desc&limit=${limit}&select=role,content,created_at`,
  );
  return (rows || []).reverse();
}
...
export async function listMemories(adminChatId, limit = 50) {
  return request(
    `${table('ai_memories')}?admin_chat_id=eq.${encodeURIComponent(adminChatId)}&order=updated_at.desc&limit=${limit}&select=id,content,created_at,updated_at`,
  );
}
