...
export async function listPending(limit = 20) {
  ...
  // SEND ALL must never inherit the legacy caller's 50-item cap. When there is
  // a recent SEND ALL anchor, snapshot the whole remaining range (up to the
  // store safety ceiling) so one click can finish the full batch.
  const n = anchor ? 1000 : safeLimit(limit);
